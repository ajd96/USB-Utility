...
@Purpose: The purpose of this python module/program is to provide the user the option to delete all the created 'grub' backup files/directorys within the Linux filesystem
          directory 'etc/default/grub_backup' which could be used to clean any left over files which are no longer required. The module/program implementation provides a simple command
          line interface (CLI) which the user can interact with by using keyboard input aswell as providing additional safeguarding features to prevent the user from deleting important
          backup data by mistake. Also the source code implementation provides additional features such as converting the input string data to lowercase and removing whitespace
          (Spaces) within string input data which reduces any module/program errors overall during usage by the user.
...
lowercaseOne = userInput.lower()
lowercaseTwo = finalUserInputWarning.lower()

# Removes all the whitespaces / spaces within the input strings.
removeallSpaces_lowercaseOne = lowercaseOne.replace(" ", "")
removeallSpaces_lowercaseTwo = lowercaseTwo.replace(" ", "")

if not removeallSpaces_lowercaseOne.isalpha() or not removeallSpaces_lowercaseTwo.isalpha():    # Checks if the user input has only alphabetical based input (String or characters only). If its not alphabetical based input, execute the condition.
    ...
    exit()  # Exit/close the python program when condition is met.

else:
    if removeallSpaces_lowercaseOne == "yes" and removeallSpaces_lowercaseTwo == "yes":     # If both user input is equals to 'yes', execute the condition.
...
if not removeallSpaces.isalpha():    # Checks if the user input has only alphabetical based input (String or characters only). If its not alphabetical based input, execute the condition.
    ...
    if removeallSpaces == "yes":    # if variable is equal to 'yes' then execute the if block.
        ...
        print("Program is closing due to input is 'no' or invalid") # Print output to terminal
