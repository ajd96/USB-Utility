...
@Purpose: The purpose of this python module/program is to provide the user the option backup their original/working version of the 'grub' file before performing any type of modifications to the file manually or automaitically using the other python modules provided in the project. The python module will essentially create a default backup directory called 'grub_backup' within the Linux filesystem `/etc/default/grub_backup` and then allow the user to backup their `grub` file within a created 'sub' directory which gets created by user input, string concatenation and Linux system command execution, the 'sub' directory will be created under the 'grub_backup' directory which will keep all 'grub' file backups organized in one location. The python module/program also contains directory based checks to make sure the user cannot duplicate any 'grub' file backups using the same 'sub' directory name, aswell as preventing the root backup directory (grub_backup) from being duplicated. The module will copy the 'grub' file from the filesystem directory '/etc/default/grub' to the desired 'sub' directory automaitcally which is created earlier within the python module.
'''
...
backupDirectoryName = raw_input("Enter the directory name you wish to save the 'grub' file too: ") # Asks and stores users 'String' input for the sub directory name. Spaces are not supported.

backupDirectoryName_SpacesRemoved = backupDirectoryName.replace(" ", "")    # Removes all spaces from the string.

createBackupDirectoryName = makeSubBackUpDirectory + "/" + backupDirectoryName_SpacesRemoved # Concatenates the final string to produce the created backup directory in which the user is going to store their 'grub' file.
...
if backupDirectoryAlreadyExist:        # Checks if the condition returns 'True' or 'False' depending if the directory already exists within the Linux filesystem and depending on the result, the if-else statement is executed.
    while True:
        backupDirectoryName = raw_input("[ERROR] Directory already exist, please enter a new directory name you wish to save the 'grub' file too: ") # Asks and stores users 'String' input for the sub directory name. Spaces are not supported.
        
        backupDirectoryName_SpacesRemoved = backupDirectoryName.replace(" ", "")    # Removes all spaces within the string
...
createBackupDirectoryNameExecution = "sudo mkdir " + makeSubBackUpDirectory + "/" + backupDirectoryName_SpacesRemoved # Performs a string concatenation with multiple strings to create the Linux execution command for creating the 'sub' backup directory.
...
pathToCustomDirectory = "/etc/default/grub_backup/" + backupDirectoryName_SpacesRemoved # Concatenates the custom created 'grub' backup directory with the users created 'sub' directory located within the root directory.
